...
bias1=zeros(1,100);
hiddenoutput=zeros(1,100)*0.01;
w2=normrnd(0,1,100,10);
bias2=zeros(1,10);
...
for a=1:1 %train time
   for i=1:60000 %train data
       ...
       for n=1:10
           ...
           gradb2(n)=gradb2(n)+(diff2(n))*(diff2(n));
           bias2(n)=bias2(n)-learnrate2*diff2(n)/(sqrt(gradb2(n))+1e-7);
       end
       for m=1:100
           diff=0.01;
           if (hiddenoutput(m)>0)
           ...
           gradb1(m)=gradb1(m)+(diff1(m))*(diff1(m));
           bias1(m)=bias1(m)-learnrate1*diff1(m)/(sqrt(gradb1(m))+1e-7);
           for j=1:784
               gradw1(j,m)=gradw1(j,m)+(diff1(m)*data(i,j))*(diff1(m)*data(i,j));
               w1(j,m)=w1(j,m)+learnrate1*diff1(m)*data(i,j)/(sqrt(gradw1(j,m))+1e-7);
           end
       end
       for n=1:10
           for k=1:100
               gradw2(k,n)=gradw2(k,n)+(diff2(n)*hiddenoutput(k))*(diff2(n)*hiddenoutput(k));
               w2(k,n)=w2(k,n)+learnrate2*diff2(n)*hiddenoutput(k)/(sqrt(gradw2(k,n))+1e-7);
           end
       end
   end
end
