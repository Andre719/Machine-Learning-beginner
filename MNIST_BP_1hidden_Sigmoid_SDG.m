...
for a=1:1 %train time
   for i=1:60000 %train data
       ...
       for m=1:100
           hiddenoutput(m)=1/(1+exp(-1*hiddenoutput(m)));
       end
       finaloutput=hiddenoutput*w2;
       finaloutput=finaloutput-bias2;
       for n=1:10
           finaloutput(n)=1/(1+exp(-1*finaloutput(n)));
       end
       result=zeros(1,10);
       result(label(i)+1)=1;
       for n=1:10
           diff2(n)=finaloutput(n)*(1-finaloutput(n))*(result(n)-finaloutput(n));
           bias2(n)=bias2(n)-learnrate2*diff2(n);
       end
       for m=1:100
           diff1(m)=w2(m,:)*diff2';
           diff1(m)=hiddenoutput(m)*(1-hiddenoutput(m))*diff1(m);
