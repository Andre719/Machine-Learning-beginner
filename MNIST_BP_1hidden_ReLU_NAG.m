...
for a=1:1 %train time
   for i=1:60000 %train data
       ...
       for n=1:10
           ...
           prev=nmb2(n);
           nmb2(n)=friction*nmb2(n)-learnrate2*diff2(n);
           bias2(n)=bias2(n)-friction*prev+(1+friction)*nmb2(n);
       end
       for m=1:100
           diff=0;
           if (hiddenoutput(m)>0)
           ...
           prev=nmb1(m);
           nmb1(m)=friction*nmb1(m)-learnrate1*diff1(m);
           bias1(m)=bias1(m)-friction*prev+(1+friction)*nmb1(m);
           for j=1:784
               prev=nmw1(j,m);
               nmw1(j,m)=friction*nmw1(j,m)+learnrate1*diff1(m)*data(i,j);
               w1(j,m)=w1(j,m)-friction*prev+(1+friction)*nmw1(j,m);
           end
       end
       for n=1:10
           for k=1:100
               prev=nmw2(k,n);
               nmw2(k,n)=friction*nmw2(k,n)+learnrate2*diff2(n)*hiddenoutput(k);
               w2(k,n)=w2(k,n)-friction*prev+(1+friction)*nmw2(k,n);
           end
       end
   end
end
